...
export default function ActionsModal({ isOpen, onClose, tooltipLabel, currentValue, onSelect, availableActions, onResetCurrent }) {
  ...
  // Categorize and filter actions - Version 2
  const categorizeActions = (actions) => {
    ...
    // Define which IDs belong to which category
    const buttonIds = ['leftStick', 'rightStick', 'leftBumper', 'rightBumper', 'leftTrigger', 'rightTrigger', 'dPadUp', 'dPadDown', 'dPadLeft', 'dPadRight', 'buttonA', 'buttonB', 'buttonX', 'buttonY'];
    const assignmentIds = ['highContrast', 'launchApp', 'magnifier', 'openGuide', 'playOrPause', 'quickSettings', 'record'];
    const systemIds = ['mute', 'volumeUp', 'volumeDown'];
    const macroIds = ['rapidFire', 'bunnyHop', 'buildWall', 'quickBuy', 'chatMacro1', 'chatMacro2'];

    actions.forEach(action => {
      if (buttonIds.includes(action.id)) {
        categories['BUTTONS'].push(action);
      } else if (assignmentIds.includes(action.id)) {
        categories['ASSIGNMENT'].push(action);
      } else if (systemIds.includes(action.id)) {
        categories['SYSTEM'].push(action);
      } else if (macroIds.includes(action.id)) {
        categories['MACROS'].push(action);
      }
    });
    ...
    <>
      ...
      >
        ...
        <div className="flex items-start justify-between p-6 pb-4">
          <div>
            <h2 className="font-logitech font-bold text-white text-sm tracking-[0.36px] leading-[1.16]">
              Actions
            </h2>
        ...
        <div className="pl-6 pr-9 pb-4">
          <div className="bg-[#2e2e2e] rounded-lg flex items-center px-3 h-10 border border-transparent focus-within:border-[#00b6fa] transition-all relative overflow-hidden group">
            {/* Hover overlay */}
            ...
            <svg className="w-4 h-4 text-[#8e8e8f] shrink-0 relative z-10" fill="none" viewBox="0 0 24 24" stroke="currentColor">
            ...
            <input
              ...
              className="flex-1 bg-transparent text-[14px] text-white placeholder:text-[#8e8e8f] outline-none ml-2 font-logitech tracking-[-0.42px] leading-[1.3] relative z-10"
            />
          </div>
        </div>
