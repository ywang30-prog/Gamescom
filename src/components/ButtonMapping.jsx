    ...
    <div className="bg-black flex flex-col h-screen w-full">
      ...
      <div className="flex-1 flex gap-4 overflow-hidden pb-8 px-8 pt-4">
        ...
        <div className="flex-1 flex flex-col">
          ...
          <div className="flex flex-col items-center justify-center" style={{ height: 'calc(100vh - 48px - 64px - 32px - 16px)', marginTop: '32px' }}>
          <div className="relative w-[1188px] h-[771px]" ref={controllerRef}>
            ...
            <HotspotMarker
              key={`LD1-${viewMode}`}
              name="LD1"
              style={{ left: viewMode === 'back' ? '183px' : '268px', top: viewMode === 'back' ? '314px' : '254px' }}
              isSelected={false}
            ...
            {/* LD2 - Second from top left dot */}
            <HotspotMarker
              key={`LD2-${viewMode}`}
              name="LD2"
              style={{ left: '173px', top: viewMode === 'back' ? '107px' : '115px' }}
              isSelected={false}
            ...
            {/* LD4 - Fourth from top left dot */}
            <HotspotMarker
              key={`LD4-${viewMode}`}
              name="LD4"
              style={{ left: viewMode === 'back' ? '195px' : '175px', top: '217px' }}
              isSelected={false}
            ...
            {/* RD2 - Second from top right dot */}
            {viewMode !== 'back' && (
              <HotspotMarker
                name="RD2"
                style={{ left: '623px', top: '167px' }}
                isSelected={false}
            ...
            {/* RD3 - Middle right dot */}
            {viewMode !== 'back' && (
              <HotspotMarker
                name="RD3"
                style={{ left: '488px', top: '166px' }}
                isSelected={false}
            ...
            <HotspotMarker
              key={`RD4-${viewMode}`}
              name="RD4"
              style={{ left: viewMode === 'back' ? '530px' : '560px', top: viewMode === 'back' ? '217px' : '223px' }}
              isSelected={false}
            ...
            {/* RD5 - Bottom right dot */}
            <HotspotMarker
              key={`RD5-${viewMode}`}
              name="RD5"
              style={{ left: viewMode === 'back' ? '543px' : '467px', top: viewMode === 'back' ? '314px' : '252px' }}
              isSelected={false}
            ...
            {showResetButton === 'leftStick' && (
              <ResetButton
                position={{ left: 27, top: viewMode === 'back' ? 300 : 465 }}
                align="right"
            ...
            {(showResetButton === (viewMode === 'back' ? 'rightTrigger' : 'dPadUp')) && (
              <ResetButton
                position={{ left: 27, top: viewMode === 'back' ? 92 : 90 }}
                align="right"
            ...
            {showResetButton === 'dPadDown' && (
              <ResetButton
                position={{ left: 26, top: viewMode === 'back' ? 203 : 273 }}
                align="right"
            ...
            {showResetButton === 'buttonA' && (
              <ResetButton
                position={{ left: 724, top: viewMode === 'back' ? 203 : 368 }}
                align="left"
            ...
            {showResetButton === 'rightStick' && (
              <ResetButton
                position={{ left: 724, top: viewMode === 'back' ? 300 : 465 }}
                align="left"
...
function HotspotTooltip({ label, value, position, isActive, isModified, align = 'right', isHoveredByDrag = false, tooltipRef, onClick, tooltipId, onHoverChange, hoveredTooltipId }) {
  ...
  return (
    <div
      ref={tooltipRef}
      className="absolute backdrop-blur-sm rounded group"
      data-tooltip
      style={{
